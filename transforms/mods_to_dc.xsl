...
-->
	...
	<xsl:template match="mods:note[@type='FOR_code']">
	...
	<xsl:template match="mods:note[@type='anzsrc_SEO_socio-economic_objective_code']">
			<dc:subject>
				<xsl:value-of select="."/>
		    </dc:subject>
	</xsl:template>
		...
		<xsl:choose>
			...
			<xsl:when test="@type='host'">
			    <xsl:for-each select="mods:originInfo">
			        ...
			        <dc:date><xsl:value-of select="mods:dateIssued"/></dc:date>
		        </xsl:for-each>		        
			    <dc:relation>
					<xsl:for-each
						select="mods:titleInfo/mods:title | mods:identifier | mods:location/mods:url">
					...
					<xsl:text> </xsl:text>
						<xsl:choose>
					<xsl:when test="mods:part/mods:detail[@type='volume'] or mods:part/mods:extent">
					...
					<xsl:for-each select="mods:part[mods:detail[@type='issue']]">
					    ...
					    <xsl:value-of select="mods:detail[@type='issue']/mods:number"/>
					</xsl:for-each>
					<xsl:for-each select="mods:part[mods:detail[@type='number']]">
					    <xsl:text> No. </xsl:text>
					    <xsl:value-of select="mods:detail[@type='number']/mods:number"/>
					</xsl:for-each>
					<xsl:for-each select="mods:part/mods:extent[mods:start | mods:end]">
					    <xsl:text> pp: </xsl:text><xsl:value-of select="mods:start"/><xsl:text>-</xsl:text><xsl:value-of select="mods:end"/>
					</xsl:for-each>
					  </xsl:when>
					  <xsl:otherwise>
