<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
	xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
	exclude-result-prefixes="xlink marc xsi" version="1.0">
...
-->
		...
		<xsl:for-each select="marc:datafield[@tag=655]">
		   <xsl:variable name="restype" select="marc:subfield[@code='a']"/>
	...
	<!--	<xsl:variable name="physicalDescription"> -->
		...
		<!--	<xsl:choose>
				...
				<!-- Altered by David as we have hard coded all leaders to electronic when they are mostly print -->
			<!--	<xsl:when
...
<!-- David commented out and replaced -->
		<!--	<xsl:for-each select="marc:datafield[@tag=300]">
			<physicalDescription>
			  <form authority="marcform">print</form>
				<extent>
...
-->
	...
	<!-- <xsl:for-each select="marc:datafield[@tag=585]">
		...
		<!-- David added -->
		<xsl:for-each select="marc:datafield[@tag=654]"> 
			<xsl:call-template name="createSubFrom654"/>
		</xsl:for-each>
...
<!-- Added by David -->		
		<xsl:for-each select="marc:datafield[@tag='088']">
			<xsl:call-template name="createIdentifierFrom088"/>
		</xsl:for-each>
...
<!-- David added 787 -->
<xsl:if test="not($restype='book')">
<xsl:if test="not($restype='Report')">
<xsl:if test="not($restype='THESIS')">
...
<relatedItem type="host">
	...
	<xsl:for-each select="marc:datafield[@tag=787]">
             <xsl:variable name="x787t">
				<xsl:value-of select="marc:subfield[@code='t']"/>
			</xsl:variable>            
             <xsl:variable name="x787g">
				<xsl:value-of select="marc:subfield[@code='g']"/>
			</xsl:variable>             
			
           <xsl:choose>
     <!-- Chapter Book Title and Citation -->						
			<xsl:when test="$restype='book chapter'">
				<xsl:if test="not($x787t='')">			
			         <titleInfo><title><xsl:value-of select="$x787t"/> </title></titleInfo>	
				</xsl:if> 
				<xsl:if test="not($x787g='')">			
			          <part><text><xsl:value-of select="$x787g"/></text></part>	
				</xsl:if> 
			</xsl:when>          
           
     <!-- Journal Title and Article Citation -->						
			<xsl:when test="$restype='journal article'">
				<xsl:if test="not($x787t='')">			
			         <titleInfo><title><xsl:value-of select="$x787t"/> </title></titleInfo>	
				</xsl:if>
				<xsl:if test="not($x787g='')">			
     ...
     <!-- Conference Proceedings Title and Conference Paper Citation -->						
			<xsl:when test="$restype='conference paper'">
				<xsl:if test="not($x787t='')">			
			         <titleInfo><title><xsl:value-of select="$x787t"/> </title></titleInfo>	
				</xsl:if>
				<xsl:if test="not($x787g='')">			
		...
		<xsl:for-each select="marc:datafield[@tag=020]">
                <xsl:variable name="x020a">
				   <xsl:value-of select="marc:subfield[@code='a']"/>
			    </xsl:variable>
			   <xsl:variable name="x020b">
				<xsl:value-of select="marc:subfield[@code='b']"/>
			</xsl:variable>
     ...
     <!-- Chapter Book ISBN -->						
					<xsl:when test="$restype='book chapter'">
                <xsl:call-template name="relatedItem76X-78X"/>
				<xsl:if test="not($x020a='')">			
			<identifier type="isbn"><xsl:value-of select="$x020a"/></identifier>	
						</xsl:if> 
				       <xsl:if test="not($x020b='')">
				         <identifier type="isbn"><xsl:value-of select="$x020b"/></identifier>
				         </xsl:if> 
					</xsl:when>
...
<!-- Conference Proceedings ISBN  -->					
 ...
 <!-- Creative Work Host Details?  -->					
					<xsl:when test="$restype='creative work'">
				<xsl:call-template name="relatedItem76X-78X"/>
				<xsl:if test="not($x020a='')">
			<identifier type="isbn"><xsl:value-of select="$x020a"/></identifier>
				</xsl:if> 
  ...
  <xsl:for-each select="marc:datafield[@tag=022]">
      ...
      <!-- Journal ISSN -->						
			<xsl:when test="$restype='journal article'">
                <xsl:call-template name="relatedItem76X-78X"/>
				<xsl:if test="not($x022a='')">			
			<identifier type="issn"><xsl:value-of select="$x022a"/></identifier>	
						</xsl:if> 
					</xsl:when>	
  ...
  <!-- David added 700e -->
  <xsl:for-each select="marc:datafield[@tag=700]">
              <xsl:variable name="x700a">
				<xsl:value-of select="marc:subfield[@code='a']"/>
			</xsl:variable>
            <xsl:variable name="x700e">
...
<!-- Chapter Book Editors -->						
				<xsl:choose>
					<xsl:when test="$restype='book chapter'">
				<xsl:if test="not($x700e='')">
				<name type="personal">
			<namePart><xsl:value-of select="$x700a"/></namePart>
			<role>
...
<!-- Journal Editors -->				                    
		  ...
		  <!-- David added 518a  and 585a -->
              <xsl:variable name="x518a">
				<xsl:value-of select="marc:datafield[@tag='518']/marc:subfield[@code='a']"/>
			</xsl:variable>
              <xsl:variable name="x585a">
				<xsl:value-of select="marc:datafield[@tag='585']/marc:subfield[@code='a']"/>
			</xsl:variable>
			<xsl:choose>
			<xsl:when test="not($x518a='') and not($x585a='')">
					<titleInfo><title><xsl:value-of select="$x585a"/>(<xsl:value-of select="$x518a"/>)</title></titleInfo>
			</xsl:when>
			<xsl:otherwise>
		        <xsl:if test="not($x518a='')">			
		            <titleInfo><title><xsl:value-of select="$x518a"/></title></titleInfo>	
		        </xsl:if> 		
		        <xsl:if test="not($x585a='')">			
		            <titleInfo><title><xsl:value-of select="$x585a"/></title></titleInfo>	
		        </xsl:if>
		</xsl:otherwise>		
			 </xsl:choose>
	...
	<!--	<xsl:for-each select="marc:subfield[@code='e']">
		...
		<!-- This is editor but wont show in edit form as editor in author list unless role=author  -->
		<xsl:for-each select="marc:subfield[@code='e']">
			<role>
				<roleTerm authority="marcrelator" type="text">Author</roleTerm>
			</role>
		</xsl:for-each>
		<xsl:if test="$x700a='' or $x700a=NULL">
		    <xsl:for-each select="marc:subfield[@code='u']">
			    <role>
				    <roleTerm authority="marcrelator" type="text">Author</roleTerm>
			    </role>
	...
	<!-- David - this is so we can see the corporate authors that were added to early marc records (generally the College and School) -->	
	<xsl:template name="role710">
		<xsl:for-each select="marc:subfield[@code='a']">		
			<role>
...
<!-- David Added -->
					<xsl:template name="specialItem76X-78X">
		       <xsl:call-template name="relatedPart"/> 
	      </xsl:template>	
	...
	<!-- David commented out old version -->
...
<!-- David commented out - Fix problem with Non-sort on title --> 			
			<!-- <xsl:choose>
				<xsl:when test="@ind2&gt;0">
					<xsl:if test="@tag!='880'">
						<nonSort>
...
<!-- David added -->
             ...
             <xsl:variable name="x700u">
				<xsl:value-of select="marc:subfield[@code='u']"/>
			</xsl:variable>	
	
		<xsl:if test="@ind1='1'">
				<xsl:if test="not($x700a=NULL) and not($x700a='') and $x700u='' and $x700e=''">
				<name type="personal">
				<xsl:call-template name="xxx880"/>
				<xsl:call-template name="nameABCDQ"/>
				<xsl:call-template name="affiliation"/>
			...
			<xsl:if test="not($x700u='') and not($x700u=NULL) and ($x700a='' or $x700a=NULL)">
				<name type="personal">
				<xsl:call-template name="xxx880"/>
				<xsl:call-template name="nameABCDQ"/>
				<xsl:call-template name="affiliation"/>
			...
			<xsl:if test="not($x700u='') and not($x700u=NULL) and not($x700a='') and not($x700a=NULL)">
				<name type="personal">
				<xsl:call-template name="xxx880"/>
				<xsl:call-template name="nameABCDQ"/>
				<xsl:call-template name="affiliation"/>
	...
	<xsl:template name="createNameFrom710">
		<name type="corporate">
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="nameABCDN"/>
			<xsl:call-template name="role710"/>
		</name>
	</xsl:template>
	...
	<!-- Changed David -->
		...
		<xsl:choose>
<xsl:when test="marc:subfield[@code='a'] = 'Report'">
         research report
 </xsl:when>
  <xsl:otherwise>
			<xsl:variable name="lowerc" select="'abcdefghijklmnopqrstuvwxyz'"/>
               <xsl:variable name="upperc" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
                    <xsl:variable name="restype_lc" select="marc:subfield[@code='a']"/>
				<xsl:value-of select="translate($restype_lc, $upperc, $lowerc)"/>
		</xsl:otherwise>			
</xsl:choose>
		</genre>
	</xsl:template>
...
<!-- David commented out - put 518a as date and location in RelatedItem=host/titleInfo/title(date:location) -->
...
-->
	...
	<xsl:template name="createNoteFrom546">
		...
		<!--        <language>
          <languageTerm authority="iso639-2b" type="code"> -->
          			<!--	<xsl:for-each select="marc:subfield[@code!='a']"> -->
          			<!--	<xsl:if test="marc:subfield[@code!='a']  ='en-aus'">eng</xsl:if>
          				<xsl:if test="marc:subfield[@code!='a']  ='eng'">eng</xsl:if> -->
                    <!--     <xsl:value-of select="."/>
                   				</xsl:for-each> -->
       <!--   </languageTerm>
        </language> -->
...
<!-- David commented out - put 585a as holding exhibition in RelatedItem=host/titleInfo/title -->
<!--	<xsl:template name="createNoteFrom585">
	...
	<xsl:template name="createSubFrom654">
				<xsl:variable name="for_code"
				select="substring-before(marc:subfield[@code='a'],',')"/>
			<xsl:variable name="for_desc"
				select="substring-after(marc:subfield[@code='a'],',')"/>
					
		<xsl:if test="@ind2=' '">
       		<xsl:for-each select="marc:subfield[@code='a']">
               <note type='FOR_code' apportionment="100">
                   <note type="anzsrc_FOR_field_of_research_code">
					<xsl:value-of select="concat($for_code,' -',$for_desc)"/>
				   </note>
    			</note>
			</xsl:for-each>
		</xsl:if>
	...
	<!-- identifier 020 024 022 028 010 037 088 UNDO Nov 23 2010 RG SM-->

	<!-- createRelatedItemFrom490 <xsl:for-each select="marc:datafield[@tag=490][@ind1=0]"> -->
	
		<xsl:template name="createIdentifierFrom088">
		<identifier type="pub_uid">
		     <xsl:value-of select="marc:subfield[@code='a']"/>
		</identifier>
			</xsl:template>
